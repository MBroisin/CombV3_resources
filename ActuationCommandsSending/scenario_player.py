...
parser.add_argument('--file', type=str, default=None, help='required, filename for output file')
parser.add_argument('-cf', "--create_file", action="store_true", help="allow creation of log file if not exist")
...
sfile = args.scenario_file

if not(sfile[-4:]=='.txt'):
    print('Unvalid format for scenario file, must be .txt')
    sys.exit()

experiment_lineup = dict()

try :
    experiment_lineup = json.load(open(sfile))
except:
